...
\IndexCustom{\FlattenName}{flatten@\emph{flatten}},
...
\LMHash{}%
It is a \Error{compile-time error} to invoke an instance method
...
\LMHash{}%
An assignable expression of the form \id{} is
analyzed statically and evaluated as
an identifier expression (\ref{identifierExpression}).

\LMHash{}%
An assignable expression of the form \code{$e$.\id} or \code{$e$?.\id}
is analyzed statically and evaluated as a property extraction
(\ref{propertyExtraction}).

\LMHash{}%
An assignable expression of the form \code{$e_1$[$e_2$]} or \code{$e_1$?[$e_2$]}
is analyzed statically and evaluated as
a method invocation (possibly conditional) of
the operator method \code{[]} on $e_1$ with argument $e_2$
(\ref{ordinaryInvocation}).

\LMHash{}%
An assignable expression of the form \code{\SUPER.\id}
is analyzed statically and evaluated as a property extraction.

\LMHash{}%
Static and evaluation of an assignable expression
of the form \code{\SUPER[$e_2$]}
is equivalent to
static analysis and evaluation of the method invocation
\code{\SUPER.[]($e_2$)}.
...
\LMHash{}%
Let $j \in 1 .. n$.
It is a \Error{compile-time error} unless $e_j$ is a constant expression.
Let $S$ be the static type of the value of $e_j$.
It is a \Error{compile-time error} unless $S$ is
a subtype of the static type of $e$.
It is a \Error{compile-time error} unless $S$ has a primitive operator \lit{==}
(\ref{theOperatorEqualsEquals}).
